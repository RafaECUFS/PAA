...
int Hashify(char* to_hash, int tamanho){
    int hash,chave, tam_str=12;
    for(int posicao=0;posicao<tam_str;posicao++){
      ((int)to_hash[posicao]%2==0)?(chave *= ((tam_str)%((int)to_hash[posicao]+1))):(chave *= ((tam_str)%((int)to_hash[posicao]+2)));
    }
...
int main(int argc, char* argv[]){
    ...
    while(index_Triagem<tam_Array_triagem-1){
        ...
        lista_Triagem[index_Triagem].indice_Lista = index_Coincide;
