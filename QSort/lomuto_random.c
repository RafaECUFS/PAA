...
int32_t Lom_Rand(int32_t* array, int32_t low, int32_t high, int32_t* num_Trocas) 
{
    ...
    Trocar(&array[high], &array[random_index], num_Trocas);    
    
    return Lom_STD(array, low, high, num_Trocas);
}
...
void quicksortLomuto(int32_t* array, int32_t ind_Menor, int32_t ind_Maior, int32_t* num_Trocas) {
    ...
    {
        (*num_Trocas)++;
        int32_t p = Lom_Rand(array, ind_Menor, ind_Maior, num_Trocas);
