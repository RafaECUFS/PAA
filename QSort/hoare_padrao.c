...
void quicksortHoare(int32_t* array, int32_t low, int32_t high, int32_t* num_Trocas) {
    (*num_Trocas)++; 
    if (low < high) {
        // ✅ conta chamada recursiva efetiva

        int32_t p = Hoa_STD(array, low, high, num_Trocas);

        quicksortHoare(array, low, p, num_Trocas);
        quicksortHoare(array, p + 1, high, num_Trocas);
    }
}
...
void processarVetor(int32_t* vetor, int32_t tamanho, FILE* output) {
    ...
    int32_t trocas = 0;
    quicksortHoare(copia, 0, tamanho - 1, &trocas);

    strcpy(variante.tipo_sort, "HP");
