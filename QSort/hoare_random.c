...
int32_t Hoa_Rand(int32_t* array, int32_t low, int32_t high, int32_t* num_Trocas) 
{


    // Escolha de pivô dentro dos limites [low, high]
    int64_t moduloValue = array[low];
    if (moduloValue < 0) moduloValue = -moduloValue;
    int32_t range = (high - low + 1);
    int32_t random_index = low + (int32_t)(moduloValue % range);
    
    Trocar(&array[low], &array[random_index], num_Trocas);    
...
void quicksortHoareRDM(int32_t* array, int32_t low, int32_t high, int32_t* num_Trocas) {
    (*num_Trocas)++; 
    if (low < high) {
        // ✅ conta chamada recursiva efetiva

        int32_t p = Hoa_Rand(array, low, high, num_Trocas);

        quicksortHoareRDM(array, low, p, num_Trocas);
        quicksortHoareRDM(array, p + 1, high, num_Trocas);
    }
}
...
void processarVetor(int32_t* vetor, int32_t tamanho, FILE* output) {
    ...
    int32_t trocas = 0;
    quicksortHoareRDM(copia, 0, tamanho - 1, &trocas);

    strcpy(variante.tipo_sort, "HA");
